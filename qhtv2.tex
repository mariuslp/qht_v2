...
\begin{definition}[Duplace detection filter]
...
Unfortunately, duplicate detection over an infinite stream has the following effect. Since a filter can only store at most one element per bit \cite[Section 2.1]{10.1145/3297280.3297335}, a filter of size $M$ can information-theoretically remember at most $M$ elements. Thus after $fM$ insertions (with $f \gg 1$), at most a small fraction $1/f \ll 1$ of the stream can be stored, we say that the filter is \emph{saturated}. 

In other terms, any filter is asymptotically as good as a random guess:
...
Let $\mathcal F$ be the following filter: the filter's state consists in a queue of $w$ hashes, and for each new element $e$, $\mathsf{Detect}(e)$ returns \DUPLICATE if $h(e)$ is present in the queue, \UNSEEN otherwise. $\mathsf{Insert}(e)$ appends $h(e)$ to the queue before popping the list. 
