...
\begin{definition}[Duplicate detection problem, DDP] Given a stream $E_n = (e_1, e_2, \dotsc, e_n)$ and a \enquote{new} item $e^\star$, find whether $e^\star \in E_n$.\footnote{
	...
	At every time increment, the new item is added to the stream, i.e., $E_{n+1} = E_n | (e^\star)$ where | denotes the concatenation . 
\end{definition}
...
A filter comparison was already made in \cite{10.1145/3297280.3297335}. Taking the same filters parameters, created another benchmark, studying the resistance of each DDF to saturation. We added the lower bound on the error rate. More details about the experiments are given in Section~\ref{sub:saturation}. We observe in Figure~\ref{fig:graph_n} that the most efficient DDF is the QHT \cite{10.1145/3297280.3297335}.

%However, as noted in Section~\ref{sub:related}, A2 filters can be considered as a queuing filter, with $L=2$ and Bloom filters as subfilters. Hence, as it is not interesting to put a queueing filter inside a queueing filter, we will rather use QHT for practical benchmarking of the queueing filter.
...
%In the experiments we use QHT for practical benchmarking of the queueing filter.
...
We used two streams. The first one was a real stream, collected from \cite{10.1145/3297280.3297335}, which is a web crawler's log, of alphabet size unknown, with about 10\% duplicates on the longest stream, consisting of 150 000 000 elements. The second one was a stream of random elements, taken from an alphabet of $2^{26}$ elements, with around 8\% duplicates on the longest stream. Results are plotted in Figure~\ref{fig:graph_n}.
...
In this section, we now focus on the security on the queueing filter. We consider an adversarial game in which the attacker wants to trigger false positives or false negatives over the sliding window. This has practical applications in several topics of security, including cache saturation or denial of service by forcing cache misses, or by making undetected fraudulent payments. \todo[source]. 
...
This adversarial game can be trivially adapted for a false positive (resp. false negative) adversarial game over a sliding window of size $w$, hence we omit its formal definition.
 
\subsection{First Approach on the Whole Sliding Window}
We first define what an adversarial false positive resistance over a sliding window is, before giving its value for any queueing filter based on the subfilter's properties.

\begin{definition}{Adversarial False Positive Resistance}
	Let $\mathcal F$ be a filter, let $E$ be a stream. We say that $\mathcal F$ is $(p, n)$-resistant to adversarial false positives if no polynomial-time probabilistic (PPT) adversary $\mathcal A$ can win the $n-$false positive adversarial game with probability more than $p$.  
\end{definition}

Note that if $\mathcal F$ is $(p,n)$-resistant, it is as well $(p, m)$-resistant for any $m < n$.

This definition can be trivially adapted for false negative resistance, and for resistance over a sliding window.

\begin{theorem}
	Let $\mathcal Q$ be a filter of $L$ subfilters $\mathcal F_i$, with $c$ insertions maximum per subfilter, let $E$ be a stream, let $w$ be a sliding window size. 
...
\begin{proof}
	Let us first assume that $cL \leq w$. This implies that information-theory wise, the elements remembered by each subfilter belong to the sliding window.
	
	The probability of false positive for $\mathcal Q$ is $1 - (1 - \FP_{\mathcal F, c})^L$. As we see, the probability of false positive in $\mathcal Q$ over the sliding window $w$ is strictly increasing with the probability of false positive in any subfilter $\mathcal F_i$. Hence, the optimal solution is to maximise the probability of false positive in each subfilter $\mathcal F_i$, which by hypothesis is bounded above by $p$ after $c$ insertions.
	
	On the other hand, if $cL > w$, it means that the oldest filter remembers elements that do not belong to the sliding window anymore. Hence, a strategy for the attacker trying to trigger a false positive on $e^\star$ could be to make it so these oldest elements are all equal to $e^\star$. Let $E$ be the optimal adversarial stream for triggering a false positive on the sliding window $w$ with the element $e^\star$, when $cL \leq w$. The adversary $\mathcal A$ can create a new stream $E' = e^\star | e^\star | \dotsc | E$ where $e^\star$ is prepended $cL - w$ times to $E$.
	
	After $w$ insertions, it is quite likely that the last subfilter will answer $\DUPLICATE$ with probability at least $p$, hence giving a lower bound on $\mathcal A's$ probability of success.
	
	Note that if, for some reason, the last subfilter answers $\DUPLICATE$ with probability less than $p$, then the default policy the same reasoning as for when $cL \leq w$ still applies, hence the lower bound (which is, in this case, an equality).
\end{proof}

\subsection{Adversarial False Negative}

\begin{theorem}
	Let $\mathcal Q$ be a filter of $L$ subfilters of kind $\mathcal F$, with $c$ insertions maximum per subfilter, let $E$ be a stream, let $w$ be a sliding window size. 
	...
	On the other hand if $w > (L-1)c$ then $\mathcal Q$ is  $(\min(1 - q, p)^{L}, w)$-resistant to false negative attacks on the sliding window $w$.
\end{theorem}
...
\begin{proof}
	...
	When $e_{Lc+1}$ is inserted, all elements $(e_1, \dotsc, e_{c-1}, e^\star)$ are dropped as the oldest subfilter is popped. Hence, in this context $e^\star$ is not in any subfilter anymore, so by adapting the previous analysis, $\mathcal A$ can get a false negative with probability at most $\min(1 - q, p)^{L}$.
\end{proof}
%% Acknowledgements 
%\begin{acks} \end{acks}
