...
Unfortunately any solution to this problem faces a phenomenon called \enquote{saturation} on large enough data streams. In fact, saturation is inevitable with infinite windows \cite{10.1145/3297280.3297335}, and when it happens the algorithm performs no better than at random. 
...
\begin{itemize}
	\item block decaying Bloom Filter (b\_DBF) \cite{She08}, sliding window of size $w$
...
It remains to decide which subfilter construction to choose. The results of an experimental comparison of different DDFs (details about the benchmark are given in Section~\ref{sub:saturation}) are summarizes in Figure~\ref{fig:graph_n}. It appears that the most efficient filter (in terms of saturation rate) is the QHT \cite{10.1145/3297280.3297335}.
