...
The sliding window variant of DDP may seem to be a slight weakening of the original problem.% Remarkably, it turns out that DDFs which perform well for the DDP do rather poorly for the sliding window DDP.
...
It can be noted that a CSHF of size $M$ has the same error rate than an SHF of size $(2 + \frac {\log_2 w}h)M$.
...
\todo[À faire]
In this section, we compare the efficiency of QHT and QHT-queuing filter. For this, we use an artificial dataset on various sliding window sizes.
