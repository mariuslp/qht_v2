...
The queueing filter's properties can be derived from the subfilters'. False positive and false negative rates are of particular interest. In this section we consider a queuing filter $\mathcal Q$ with $L$ subfilters $\mathcal F_i$ and 
capacity $c$ (which means that the last subfilter is dropeed after $c$ insertions).
...
\begin{theorem}
	...
	where $\bmod$ is the modulo operator and 
	 $\FP_{\mathcal F,m}$ be the probability of false positive of any of $\mathcal Q$'s subfilters $\mathcal F_i$ after $m$ insertions.
\end{theorem}
...
\begin{theorem}
	...
	\begin{align*}
	\FN_{\mathcal Q, m}^w 
	= & \left[p_c\FN_{\mathcal F,c} + \left(1-p_c\right)\left(1-\FP_{\mathcal F,c}\right)\right]^{L-1} \\
	 \times & \left[p_{m\bmod c}\FN_{\mathcal F,m\bmod c} + \left(1-p_{m\bmod c}\right)\left(1-\FP_{\mathcal F,m\bmod c}\right)\right]
	\end{align*}
	where $\bmod$ is the modulo operator, $\FN_{\mathcal F,m}$ is the probability of false negative of any subfilter $\mathcal F$ after $m$ insertions, and
	\begin{equation*}
		p_m = \frac{1-\left(1-\frac 1{|\Gamma|}\right)^m}{1-\left(1-\frac 1{|\Gamma|}\right)^w}.
	\end{equation*}
\end{theorem}
...
\begin{align*}
	...
	& = 1 -  (1 - \FP_{\mathcal F, c})^{L-1}\sum_{\ell = 0}^{c-1} \FP_{\mathcal F, \ell}
\end{align*}
...
The stream was a stream of uniform random elements, taken from an alphabet of $2^{26}$ elements, and amounts for about 8\% duplicates on the 150 000 000 elements in the longest stream used. Results are plotted in Figure~\ref{fig:graph_n}.
