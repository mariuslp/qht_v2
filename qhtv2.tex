...
We introduce a variant of the DDP over a sliding window, which enables the design of filters that avoids the saturation issue identified in \cite{10.1145/3297280.3297335} (Section~\ref{sec:filsat}). We provide explicit upper bounds, and corresponding constructions, for the amount of memory necessary to solve this problem (Section~\ref{sub:opt_solutions}), then introduce the \emph{queuing construction} as a generic approach to build windowed-DDP-friendly filters from existing filters (Sections~\ref{sec:queue} and \ref{sec:subfilter-select}). We analyse the queuing construction's error rates and parameters (Section~\ref{sec:optimising}), which are confirmed by benchmarks (Section~\ref{sec:bench}). Finally, we provide an analysis of the queueing construction's resistance to adversarial streams (Section~\ref{sec:adversarial}).
...
\section{Queuing filters from existing DDFs}\label{sec:subfilter-select}
...
\subsection{How many subfilters?}\label{sub:L_bench}
\todo[À faire]
...
\section{Effects of the finite simulation}
