...
However, this implementation depends on $\log_2 |\Gamma|$, which might be arbitrarily large. We thus propose another solution, still in constant time but with a with a very low error rate, and not depending on $|\Gamma|$.
...
\begin{theorem}
	...
	If $M \simeq 5w\log_2 w$, then it is possible to upgrade this filter so its error rates remain the same but the complexity drops to constant time.
\end{theorem}
...
Note that this solution has a time complexity of $O(w)$. Using an additional dictionary like in the previous proof, we get a filter with an error rate of about $\frac 1w$ and constant time for insertion and lookup, but requiring $5w\log_2 w$ bits of memory.
\end{proof}
