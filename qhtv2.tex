...
The notion of sliding window was, as far as we know, first introduced in \cite{10.1145/1060745.1060753}. However, several variations exist and describe incompatible situations, such as \cite{shtul2020agepartitioned}. Instead, the wDDP formulation we rely on is due to \cite{Yoo10,She08}, which also introduce algorithms for solving the wDDP approximately.

The notion of using subfilters, as in the queuing construction, can be found in the A2 filter's design \cite{Yoo10} and a variation thereof can be found in \cite{shtul2020agepartitioned} but in a different DDP formulation. The A2 is built from two Bloom filters, a situation which we generalise and analyse generically in this paper. Similarly, the construction in \cite{shtul2020agepartitioned} only works with Bloom Filters.
...
We usually consider the situations where the available memory is too small for perfect detection, i.e., $|\mathcal S| \ll |\Gamma|$. Otherwise, if $|\mathcal S| = |\Gamma|$ then the problem can be solved in constant time without errors \cite{10.1145/3297280.3297335}.
...
We have $\FPR^w_n = \frac{1}{n}\sum_{i=1}^n \FP^w_i$, and similarly for $\FNR^w_n$.
...
Removing the dictionary from the SHF construction yields a more memory-efficient, but less time-efficient construction, which we dub \enquote{compact} short hash filter (CSHF). The CSHF performs in linear time in $w$ and is described in Algorithm~\ref{alg:CSHF}.
...
\begin{theorem}	
	\begin{align*}
		\FN_\text{SHF}^w&  = \FN_\text{CSHF}^w =0 \\
		\FP_\text{SHF}^w & = 1 - \left(1 - \sqrt{w2^{-M/w}}\right)^w \\
		\FP_\text{CSHF}^w & = 1 - \left(1 - 2^{-M/w}\right)^w
	\end{align*}
\end{theorem}
...
Both SHF and CSHF have strictly increasing error rates, which reach a threshold of $1/2$ for some maximum window size $w_\text{max}$. Beyond this value, these filters saturate extremely quickly: in other words, most SHF will either have an error rate of $0$ or $1$.

An illustration of this phenomenon can be seen in Figure~\ref{fig:shorts}, 
which shows the error rates for CSHF and SHF with $M=100000$, against 
...
\begin{figure}[!ht]
	...
	\caption{Error rates of SHFs and CSHFs for $M = 10^5$ bits, for various window sizes $w$.}\label{fig:shorts}
\end{figure}

The value $w_\text{max}$ can be obtained by solving (numerically) for $\FP^{w_\text{max}} = 1/2$ for a given $M$. Experimentally, we observed an approximately linear relationship between $M$ and $w_\text{max}$ for large values of $M$. 
...
\subsection{Performance in wDDF}
...
Remarkably, some of these filters still outperform dedicated windowed filters for some window sizes at least, as shown in Figure~\label{fig:improvement}. Nevertheless, we will now discuss the queuing construction, which allow us to build windowed filters from these.  
