...
Finally, we provide an analysis of our queueing construction's resistance to adversarial streams. (Section~\ref{sec:adversarial}).
...
indicate an approximately linear relationship between $M$ and $w_\text{max}$: %for large values of $M$:
 $w_\text{max}^\text{CSHF} = 0.0627 M + 443$ ($r^2 = 0.9981$) and $w_\text{max}^\text{SHF} = 0.0233 M + 186$ ($r^2 = 0.9977$).
...
\begin{theorem}
	...
	for any $n > M$.
\end{theorem}
In particular, the asymptotic error rate $E_\infty$ satisfies $E_\infty \geq \left(1 - \frac{1}{|\Gamma|}\right)^{M} \approx 1 - M/|\Gamma|$.
