...
Let $E = \{e_i\}$ be a stream over $\Gamma$. Some element $e_j \in E$ is called a \emph{duplicate} if and only if there exists $i < j$ such that $e_i = e_j$. Otherwise, $e_j$ is said to be \emph{unseen}.
\end{definition}
...
\begin{definition}[False positive, false negative]
	Let $e \in E$ be an unseen (resp. duplicate) and $\mathcal F$ be a filter. We say that $e$ is a \emph{false positive} (resp. \emph{false negative}) when $\mathcal F.\mathsf{Detect}(e) = \DUPLICATE$ (resp. $\UNSEEN$).
\end{definition}
