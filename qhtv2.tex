...
\title{Optimal Duplicate Detection in a Sliding Window}
...
\begin{abstract}
	...
	Building on the Bloom filter \cite{bloom_spacetime_1970}, an early data structure amenable to duplicate detection, several generalisations and variants were proposed tthat reduce error rates while maintaining constant memory. 
	
	Unfortunately the infinite stream setting is ill-posed, and error rates of duplicate detection filters turn out to be heavily constrained: as a consequence they appear to provide no advantage, asymptotically, over a random biased coin toss \cite{10.1145/3297280.3297335}.
...
On a side note, it is clear that the input distribution plays a central role regarding how efficiently the DDP can be solved. For instance, some deterministic streams may be expressed very compactly (such as the output of a PRNG with known seed) making the DDP relatively easy. Information-theoretically, if the source has $U$ bits of entropy then the situation is equivalent to having an $U$-bit, uniformly distributed input. This is the setting we consider here.

As the window size in wDDP grows infinitely large, it becomes the following problem: find whether $e^\star \in E_n$. This \enquote{non-windowed} problem predates in fact the definition of wDDP, and several algorithms have been proposed for this problem: \cite{Den06,10.1145/3297280.3297335,Dut13} and a slight variation of Cuckoo Filters \cite{Fan14} suggested by \cite{10.1145/3297280.3297335}. Unfortunately \emph{all} these solutions face a phenomenon called \enquote{saturation} on large enough data streams. In fact, saturation is inevitable with infinite windows \cite{10.1145/3297280.3297335}, and when it happens the algorithm performs no better than at random. 

This is problematic on two grounds: it makes the comparison of several algorithms difficult (since they all asymptotically behave in that fashion), and the unavoidable saturation ruins any particular design's merits.  

\subsection{Contributions}
In this paper we show that the wDDP problem can be solved exactly using $M$ bits of memory, up to a window size $w_\text{max}$; we provide an explicit algorithm achieving this, which can run in constant time. Allowing small errors makes it possible to push $w_\text{max}$ further; we provide the corresponding constant-time (SHF) and linear-time (CSHF) constructions.

Unfortunately, for $w > w_\text{max}$ the performance of these algorithms degrade very rapidly. We therefore turn our attention to existing data structures designed for the \enquote{non-windowed} setting. We show that some of them outperform dedicated data structures, including SHF/CSHF, in the $w > w_\text{max}$ regime.
...
Finally, we provide an analysis of our construction's resistance to adversarial streams (Section~\ref{sec:adversarial}).


\subsection{Related work}\label{sub:related}
The notion of sliding window was, as far as we know, first introduced in \cite{10.1145/1060745.1060753}. However, the definition is not the one we use, as several variations exist such as \cite{shtul2020agepartitioned}. Instead, the wDDP formulation we used was already introduced in \cite{Yoo10,She08}. Both of these papers propose an algorithm for the wDDP.

Closer to our work, the idea of using subfilters can be found in the A2 filter's design \cite{Yoo10} and a variation of our idea can be found in a very recent construction, on a slightly different DDP formulation \cite{shtul2020agepartitioned}. The A2 is built from two Bloom filters, a situation which we generalise and analyse generically in this paper. Similarly, the solution in \cite{shtul2020agepartitioned} only works with Bloom Filters.
...
We usually consider the situations where the available memory is too small for perfect detection, i.e., $|\mathcal S| \ll |\Gamma|$. Otherwise, if $|\mathcal S| = |\Gamma|$ then solutions in constant time and without errors exist \cite{10.1145/3297280.3297335}.
...
\begin{definition}[Probability of false positive, of false negative]
	\label{def:pfp-pfn}
	Let $E = (e_1, \dotsc)$ be a stream, $w\in \mathbb N$, $\mathcal F$ be a filter. The \emph{probability of false positive} (resp. \emph{probability of false negative}) of $\mathcal F$ after $i$ elements over a sliding window of size $w$, denoted $\FP^w_i$ (resp. $\FN^w_i$), is defined as the probability that $e_{i+1}$ is a false positive (resp. false negative) over $w$.
\end{definition}
...
One can observe that $\FPR^w_n = \frac{1}{n}\sum_{i=1}^n \FP^w_i$, and similarly for $\FNR^w_n$.
...
\subsection{DDP vs wDDP}
As noted before, some DDF were defined on the DDP, thus not offering a simple analysis of their error rates on the wDDP. 

The sliding window variant of DDP may seem to be a slight weakening of the original problem. However, it is more appropriated for many use cases, such as caching or click fraud. % Remarkably, it turns out that DDFs which perform well for the DDP do rather poorly for the sliding window DDP.
A literature review collects the following DDF constructions: A2 filters \cite{Yoo10}, Stable Bloom Filters (SBF) \cite{Den06}, Quotient Hash Tables (QHT) \cite{10.1145/3297280.3297335}, Streaming Quotient Filters (SQF) \cite{Dut13}, Block-decaying Bloom Filters (b\_DBF) \cite{She08}, and a slight variation of Cuckoo Filters \cite{Fan14} suggested by \cite{10.1145/3297280.3297335}. The structure proposed in \cite{10.1145/1060745.1060753} is not desined for either wDDP or DDP, as they work on what they call `landmark` sliding window, which consists of a zero-resetting of the memory at some user-defined epochs.

Of these, only the A2 and b\_DBF were designed for the wDDP. The others do not account for a finite-length sliding window, and therefore cannot be adjusted as a function of $w$. Another remarkable (but experimental) observation is that these DDFs' false positive rate does not decrease to 0 when the sliding window becomes small.


\section{Optimal and Approximate Solutions}
...
\begin{proof}
	...
	Finally this filter does not make any mistake, as the dictionary $D$ keeps an exact account of how many times each element was present in the sliding window.
\end{proof}

\subsection{Approximate Optimal wDDF}
In the previous section, the optimal filter requires to know the size of $\Gamma$, which might be unknown, unbounded or simply too big for practical applications. 
The dependence on $\log_2 |\Gamma|$ can be dropped, at the cost of allowing errors.
...
\begin{theorem}\label{thm:SHF}
	...
	More precisely, it is possible to create a filter of $M$ bits with an FN of $0$, and an FP of $1 - (1-\frac1{w^2})^w \sim \frac 1w$, and a time complexity of $O(w)$.
	
	Using $M \simeq 5w\log_2 w$ bits of memory, a constant-time filter with the same error rate can be constructed.
\end{theorem}
Note that we only consider the probability of false positive after the filter has inserted at least $w$ elements, i.e., once the filter is full and has reached the stationary phase.
...
\begin{algorithm}[]
	...
	\begin{algorithmic}[1]
		...
		\State $D \gets \emptyset$ \Comment $D$ is a dictionary $h$ => counter (of max value $w$)
...
For the sake of completeness, we also introduce the Compact Short Hash Filter, which is very similar to the Short Hash Filter, but does not use any dictionnary. Hence, the filter is much slower (complexity of $O(w)$), but also has fewer errors since there is more place for the queue.

A quick description of the Compact Short Hash Filter (CSHF) Setup function is given in Algorithm~\ref{alg:CSHF}. The other functions are similar to the ones of Algorithm~\ref{alg:SHF}, removing all references to the dictionary.

\begin{algorithm}[]
	\caption{CSHF, Setup}\label{alg:CSHF}
...
%	\begin{algorithmic}[1]
%		\Function{Insert}{$e$}
%		\Comment Same as SHF, but removing all instructions about its dictionnary $D$.
%		\EndFunction
%	\end{algorithmic}
\end{algorithm}
...
\begin{theorem}
	Let $w \in \mathbb N$. Let $M \in \mathbb N$ be the available memory size. Let $\FN^w_{SHF}$ be the probability of false negative of an SHF with these parameters. We similarly define $\FP^w_{SHF}$, $\FN^w_{CSHF}$, $\FP^w_{CSHF}$. 
	Both SHF and QSHF have $\FN^w = 0$.
	
	Furthermore we have $\FP_{SHF}^w = 1 - \left(1 - \sqrt{w2^{-M/w}}\right)^w$ and $\FP_{CSHF}^w = 1 - \left(1 - 2^{-M/w}\right)^w$.
\end{theorem}
...
It can be noted that a CSHF of size $M$ has the same error rate than an SHF of size $2M + w\log_2 w$.
\subsection{Saturation}
When benchmarking both structures, it appears they saturate very quickly when changing $w$: in other words, most SHF will either have an error rate of $0$ or $1$.

For instance, we benchmarked the performance of CSHF and SHF with a memory $M=100000$, on a stream of size 1 000 000 consisting of uniformly random elements from an alphabet of size $2^{18}$. Results are given in Figure~\ref{fig:shorts}.

\begin{figure}
	\input{graphs/bench_shorts.tex}
	\caption{Error rates of SHFs and CSHFs of $100000$ bits, for various $w$.}\label{fig:shorts}
\end{figure}

Finding an exact formula of the $w_{max}$ for which those filters reach an error of 50\% proved to be tedious. However, we numerically solved the equation $\FP^w = 0.5$ for these two filters, for about 200 different values of $M$, uniformly distributed \emph{on a log scale} between $10^2$ and $10^6$. Results are given in Figure~\ref{fig:w_opt}. As it turns out, the relation between $w_{max}$ and $M$ is quasi-linear, and we get the following linear regressions, with values rounded to the third significant digit:


\begin{figure}
	\input{graphs/w_opt_shf.tex}
	\caption{Value $w_{max}$ for which SHF and CSHF reach an error of 0.5 ($\pm 0.001$), for various values of $M$.}\label{fig:w_opt}
\end{figure}


\begin{itemize}
	\item For CSHF, $w_{max}^{CSHF} = 0.0627 M + 443$, with an $r^2 = 0.9981$
	\item For SHF, $w_{max}^{SHF} = 0.0233 M + 186$, with an $r^2 = 0.9977$
\end{itemize}
...
& \quad \quad \times\left(\beta + \FN_{\mathcal F, c} (-1+x)\right) \left(\log\left(\beta + \FN_{\mathcal F, c} (-1+x)\right)-\log(x)\right) \\
&\left.\vphantom{\left(\FN^{\FN}\right)}\right] = 0
\end{align*}

If solving the equation for individual cases is doable, it however does not seem possible to derive a generic formula.
%\end{equation*}
...
\begin{figure}
	...
	\input{er_n_artificial}
	\end{tikzpicture}
	\caption{Error rate (times 100) of DDFs of 1Mb, over an artificial stream, depending on the size of the stream, without any sliding window. Hatched area represents over-optimal (impossible) values.}\label{fig:graph_n}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Error rate (times 100) of various queuing filters on various sliding window sizes, $M=10^5$, $L=10$, $|\Gamma| = 2^{18}$, on a stream of size $10^7$.}\label{tab:comparing_queuing}
\end{figure}
%\begin{table}[]
...
We now compare the efficiency of the queueing filter, depending on the number of subfilters $L$. For this, we used a queueing QQHTD, on an artificial dataset of alphabet size $\Gamma = 2^{16}$, with 100 000 elements in the stream, on various sliding window sizes.

Results are displayed in Figure~\ref{fig:bench_l}.

\begin{figure}
	\input{graphs/bench_l.tex}
	\caption{Evolution of the error rate of a queueing QHT depending on the $L$ value, for various sliding window sizes, with $M=10^5$, $|\Gamma| = 2^{18}$, on a stream of size $10^6$.}\label{fig:bench_l}
\end{figure}

We observe that the optimal value for $L$ does indeed depend on the desired sliding window. However, other experiments on alphabets of other sizes yield very similar results, hence validating the observation made in Section~\ref{sec:optimising} that the optimal number of subfilters does not depends on the alphabet (in first approximation).

\subsection{Comparison of QHT and QHT-Queuing Filter}
In this section, we compare the efficiency of QHT and QHT-queuing filter. For this, we used the same artificial dataset as previously, as well as the same QHT filter than in previous sections. Moreover, we add a queuing QHT with an optimal value $L$ for each considered sliding window.
For the sake of completeness, we also ran the comparison on the other DDFs. Results on the QHT and SQF are shown in Figure~\ref{fig:improvement}, results for the Cuckoo and SBF are shown in Appendix~\ref{app:improvement}.

\begin{figure}
	\input{graphs/improvement.tex}
	\caption{Comparing performances of various filters, in `vanilla' setting or when placed in our queueing structure.}\label{fig:improvement}
\end{figure}
...
\section{Comparing DDFs and their Queueing Counterpart}\label{app:improvement}
...
\begin{figure*}[!ht]
\begin{tikzpicture}
	\begin{semilogxaxis}[legend cell align = left, legend pos=outer north east, xlabel=$w$, ylabel=$\FPR^w + \FNR^w (\times 100)$]
...
\end{tikzpicture}
	\caption{Comparing performances of various filters, in `vanilla' setting or when placed in our queueing structure.}\label{fig:appendix_improvement}
\end{figure*}
\end{document}
