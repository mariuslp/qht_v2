...
%\subsection{imal wDDF}\label{sub:opt_solutions}
%
...
\begin{theorem}\label{thm:opt_wddf}
	For $M \geq w (\log_2(w) + 2\log_2(|\Gamma|))$, the wDDP can be solved exactly (with no errors) in constant time.
\end{theorem}
...
\subsection{Short Hash Filter and Compact Short Hash Filter Algorithms}
\paragraph{Short Hash Filter (SHF)}
...
This gives us the Short Hash Filter (SHF), described in Algorithm~\ref{alg:SHF}. The implementation relies on a double-ended queue or a ring buffer, which allows pushing at beginning of a queue and popping at the end in constant time. %A variant is described in Appendix~\ref{app:CSHF}.
...
\paragraph{Compact Short Hash Filter (CSHF)}
...
\begin{theorem}\label{thm:fprSHF}
	We have:
	\begin{itemize}
		\item $\FN_\text{SHF}^w  = 0$  and $\FP_\text{SHF}^w = 1 - \left(1 - \sqrt{w2^{-M/w}}\right)^w$
		\item$\FN_\text{CSHF}^w  = 0$  and $\FP_\text{CSHF}^w = 1 - \left(1 - 2^{-M/w}\right)^w$
	\end{itemize}	
\end{theorem}
...
\paragraph{Remark:} A CSHF of size $M$ on a sliding window $w$ has the same error rate than an SHF of sliding window $w$ of size $2M + w\log_2 w$.
...
\begin{theorem}\label{thm:asymptoticSaturation}
...
$$
	for any $n > M$.
\end{theorem}
In particular, the asymptotic error rate $EP_\infty$ satisfies 
$$
...
\begin{proof}
	...
	&=  \left(1 - \frac{1 - \Pr[\bar C]}{1 - \Pr[\bar D]}\right)(1-p)\\
	...
	EP_n & = 1 - \frac{1-\left(1 - \frac{1}{|\Gamma|}\right)^{M}}{1 - \left(1 - \frac 1{|\Gamma|}\right)^n}(1-p),
	\end{align*}
...
Hence, if $w < cL$, the queuing filter can hold information about \emph{more than $w$ elements}.
...
\begin{theorem}[Bound on false positive resistance]
...
\begin{theorem}[Bounds on false negative resistance]
